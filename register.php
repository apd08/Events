        ...
        if(isset($_POST["captcha"]) && !empty($captcha)){
            ...
            if(isset($_POST["email"])){
                ...
                if(!empty($email)){
                    ...
                    $result_query = $mysqli->query("SELECT `Email` FROM `Пользователи` WHERE `email`='".$email."'");
            ...
            $insert_role = $mysqli->query("INSERT INTO `Роль` (Тип_роли) VALUES ('user')");
            ...
            $role_id = $mysqli->insert_id;
            $result_query_insert = $mysqli->query("INSERT INTO `Пользователи` (Имя, Фамилия, Email, Пароль, Роль_FK) VALUES ('".$first_name."', '".$last_name."', '".$email."', '".$password."', '".$role_id."')");
